...
lyme_pop |> 
  ...
  ggplot(aes(x = year, y = cases_per100k, group = state, fill = state)) +
  geom_area() +
  scale_fill_manual(values = c("maroon4")) +
  scale_x_discrete(expand = c(0, NA)) + # Makes it so the data isnt just floating in the graph 
  # Expand arguement expects a vector of values
...
# stacked area chart ----
lyme_pop |> 
  filter(state %in% c("Maine", "Rhode Island", "New Hampshire", "Illinois")) |> 
  ggplot(aes(x = year, y = cases_per100k, group = state, fill = state)) +
  geom_area()
```
```{r}
# proportional stacked area chart ----
lyme_pop |> 
  filter(state %in% c("Illinois", "Rhode Island", "New Hampshire", "Vermont")) |> 
  ggplot(aes(x = year, y = cases_per100k, group = state, fill = state)) +

...
# Historan represents distribution of a numeric variable, basically it put things into "bins"
# Here we are piping data in:
...
```{r}
mko_clean %>% 
  mutate(month_name = factor(month_name, levels = month.name)) %>%# This makes it so the months are in order
  ggplot(aes(x = Temp_bot, fill = month_name)) +
  geom_histogram(postion = "identity") # This makes it so the bars are not stacked
```
...
mko_clean |> 
  ...
  geom_histogram() +
  facet_wrap(~month_name) # This will cause each month to have its own panel 
# ~ split apart our data, ~ means by
```
...
# Data wrangling with the ggplot
...
ggplot(mko_clean, aes(x = Temp_bot, y = month_name, fill = after_stat(x))) +
  ...
  scale_fill_gradientn(colors = c("#2C5374","#849BB4", "#D9E7EC", "#EF8080", "#8B3A3A")) # Scale function change how things manifest, this adjusts order of y variable

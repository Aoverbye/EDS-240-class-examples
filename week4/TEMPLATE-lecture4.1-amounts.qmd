...
jobs_clean %>% 
  ...
  geom_col() + # what type of scale are you modifying? refer back to og aesthetics mapping
...
jobs_clean |> 
  ...
  scale_y_continuous(labels = label_currency(accuracy = 1, scale = .001, suffix = "k"),
                     limits = c(0, 250000)) +
  coord_flip()
...
ggplot(analysts, aes(x = year, y = occupation, fill = total_earnings)) +
  geom_tile() +
  labs(fill = "Total_earnings") +
  coord_fixed() +
  scale_fill_viridis_c(labels = label_currency(scale = 00.1, suffix = "k")) +
  scale_y_discrete(labels = label_wrap(30)) +
  guides(fill = guide_colourbar(barwidth = 15, barheight = 0.75, title.position = "top")) +
  theme_minimal() +
  theme(
    legend.position = "top",
